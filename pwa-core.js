const PWA_CONFIG = {
  APP_NAME:    'BÁO CÁO KIỂM TRA - GIÁM SÁT',
  APP_DESC:    'Ứng dụng tạo các báo cáo kiểm tra, giám sát',
...
.pwa-dismiss{border:1px solid rgba(0,0,0,.12);background:rgba(0,0,0,.04);color:rgba(0,0,0,.7);}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('pwa-overlay').addEventListener('click', (e) => { 
    if(e.target === document.getElementById('pwa-overlay')) hideOverlay(); 
  });
  ...
  // ============= HIỂN THỊ OVERLAY CHO TẤT CẢ THIẾT BỊ =============
  // Bỏ kiểm tra tablet, tất cả Android đều hiển thị overlay
  if (!isStandalone) {
    if (!isIOS() && 'serviceWorker' in navigator) {
      // Tất cả Android (cả điện thoại và tablet) đều hiển thị overlay
      setTimeout(() => { 
        showOverlay('android'); 
